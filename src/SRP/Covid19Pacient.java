...
//	}
	...
	double calcCovid19Impact() {
		double impact;
		Afection af = new Afection(sintomak);
		double afection = af.getAfection();
...
//		afection=afection/(cardios.size()+neuros.size()+respirs.size());
		//calculate increment
		Increment p = new Increment(this.getYears(),afection);
		double increment = p.getEmaitza();
		//calculate impact
		if(this.getYears()>10) {
			 impact=increment+afection;
		}else {
			 impact=0;
		}
		return impact;
